...
const csvPath = path.join(__dirname, '..', 'page', 'jobs_data.csv');
const records = parse(fs.readFileSync(csvPath, 'utf-8'), {
    columns: true,
    skip_empty_lines: true
});

test('สร้าง Job อัตโนมัติจาก CSV - 2 Movements', async ({ page }) => {
    
    await page.goto('https://uat.thecoachcompany.co.uk/admin/');
    test.setTimeout(records.length * 180000);
    let userProfile;
   
    // --- Dialog handler (จัดการ Alert อัตโนมัติ) ---
    ...
    for (const [index, record] of records.entries()) {
        const i = index + 1;
        console.log(`🚀 [รอบที่ ${i}] กำลังสร้าง Job: ${record.emailsubject}`);
    
        // ---  LOGIN  ---
            if (i === 1) {
            await page.getByPlaceholder(/Username/i).fill(record.login_user); 
            await page.getByPlaceholder('Please enter Password', { exact: true }).fill(record.login_pass); 
            await page.getByRole('button', { name: 'LOG IN' }).click();
            userProfile = page.locator('span').filter({ hasText: record.login_user }).first();
            await expect(userProfile).toBeVisible({ timeout: 30000 });
            console.log(`✅ Login สำเร็จในชื่อ: ${record.login_user}`);
        }
        console.log(`🚀 [รอบที่ ${i}] กำลังสร้าง Job: ${record.emailsubject}`);
        ...
        await expect(async () => {
            ...
            if (!listContent.includes(record.journey_type)) {
                throw new Error(`Data [${record.journey_type}] not arrived yet`);
            }
        ...
        await suggestionItem.waitFor({ state: 'visible', timeout: 15000 });
        await suggestionItem.click();
        await page.keyboard.press('Enter');
        // --- Autofill Check ---
        await expect(page.locator('#email')).not.toBeEmpty({ timeout: 15000 });
        ...
        // SweetAlert "Do you want to change?"
        ...
        // --- Save and Send ---
        console.log(`📧 [รอบ ${i}] กำลังบันทึกและส่งอีเมล...`);
        ...
        const subjectField = emailModal.locator('#subject, input[name="subject"]').first();
        await subjectField.waitFor({ state: 'visible' });
        await subjectField.fill(`${record.emailsubject} Round ${i}`); 

        const emailIframe = page.frameLocator('iframe[title*="singleText"]').locator('body');
        await emailIframe.waitFor({ state: 'visible' });
        await emailIframe.fill(`${record.email_content} Round ${i}`);
    ...
    await reasonSelect.dispatchEvent('input');3
